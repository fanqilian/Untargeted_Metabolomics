...
#polarity = 'positive'
#workspace = '/Media/E/zhangpeng/TCM_Workspace/Project06/POS_mzXML'
#
polarity = 'positive'
workspace = '/Media/E/xingleng/005_HQT_S_7/POS_mzXML/'

#polarity = 'negative'
#workspace = '/Media/E/xingleng/005_HQT_S_6/NEG_mzXML/'
...
Peak_Filter <- function(workspace, polarity){
	...
	eicmax = NULL
	is.plotEIC.pg = F
	xsetc@rt$corrected <- xsetc@rt$raw
	AA <- xsetc@peaks
	print('++++++++++++++++++++ttt+++++++++++++++++')
	AB =na.omit(AA)
	print(head(AB))
	...
	xset2 <- group(xsetc, bw =1 , mzwid = 0.015, minfrac = para.set$minfrac)
	groupmat <- xcms::groups(xset2)
	print(groupval(xset2, "medret", "into"))
	...
	peak.table[is.na(peak.table)] <- 0
	fn.final <- paste0(workspace, '/PeakTable-FillGaps_peakss.csv')
	...
	peak.table.anno[is.na(peak.table.anno)] <- 0
	fn.final <- paste0(workspace, 'PeakTable-Annotated_peakss.csv')
	write.csv(peak.table.anno, fn.final, row.names = TRUE)
	fn.final.txt <- paste0(workspace, 'PeakTable-Annotated_peakss_v1.txt')
        write.table(peak.table.anno, fn.final.txt, row.names = F ,sep='\t')
}



Peak_Filter(workspace, polarity)
